...
def m_results(df, mean_pred, median_pred, multiple_name, value_driver):
    
    new_df = df[['{}'.format(multiple_name)]].copy().rename(columns={'{}'.format(multiple_name):'true {}'.format(multiple_name)})
    new_df['mean est pred'] = mean_pred
    new_df['median est pred'] = median_pred
    new_df['{}'.format(value_driver)] = df['{}'.format(value_driver)]
    new_df['current price'] = df['closePriceAdj']
    new_df['est price by mean'] = new_df['mean est pred'] * new_df['{}'.format(value_driver)]
    new_df['est price by median'] = new_df['median est pred'] * new_df['{}'.format(value_driver)]
    new_df['val err mean'] = (new_df['est price by mean'] - new_df['current price'])/ new_df['current price']
    new_df['val err median'] = (new_df['est price by median'] - new_df['current price'])/ new_df['current price']
    new_df['abs val err mean'] = new_df['val err mean'].abs()
    new_df['abs val err median'] = new_df['val err median'].abs()
    
    return new_df


def m_interim_accuracy_df(results_df, multiple_name, index_name1, index_name2, mean_err_name, median_err_name):
    
    total_peers_num = len(results_df)
    
    within_10_perc_val_num = len(results_df[results_df['{}'.format(mean_err_name)] < 0.10])
    within_25_perc_val_num = len(results_df[results_df['{}'.format(mean_err_name)] < 0.25])
    within_50_perc_val_num = len(results_df[results_df['{}'.format(mean_err_name)] < 0.50])
    within_75_perc_val_num = len(results_df[results_df['{}'.format(mean_err_name)] < 0.75])
    within_90_perc_val_num = len(results_df[results_df['{}'.format(mean_err_name)] < 0.90])
    ...
    accuracy_df = pd.DataFrame(accuracy_dict, index=[index_name1 + ' ' + multiple_name])
    
    
    med_within_10_perc_val_num = len(results_df[results_df['{}'.format(median_err_name)] < 0.10])
    med_within_25_perc_val_num = len(results_df[results_df['{}'.format(median_err_name)] < 0.25])
    med_within_50_perc_val_num = len(results_df[results_df['{}'.format(median_err_name)] < 0.50])
    med_within_75_perc_val_num = len(results_df[results_df['{}'.format(median_err_name)] < 0.75])
    med_within_90_perc_val_num = len(results_df[results_df['{}'.format(median_err_name)] < 0.90])
    
    med_within_10_perc_val_perc = (med_within_10_perc_val_num / total_peers_num)*100
    med_within_25_perc_val_perc = (med_within_25_perc_val_num / total_peers_num)*100
    med_within_50_perc_val_perc = (med_within_50_perc_val_num / total_peers_num)*100
    med_within_75_perc_val_perc = (med_within_75_perc_val_num / total_peers_num)*100
    med_within_90_perc_val_perc = (med_within_90_perc_val_num / total_peers_num)*100
    
    med_accuracy_dict = {
        '% of valuations within 10% of actual price':[round(med_within_10_perc_val_perc, 2)],
        '% of valuations within 25% of actual price':[round(med_within_25_perc_val_perc, 2)],
        '% of valuations within 50% of actual price':[round(med_within_50_perc_val_perc, 2)],
        '% of valuations within 75% of actual price':[round(med_within_75_perc_val_perc, 2)],
        '% of valuations within 90% of actual price':[round(med_within_90_perc_val_perc, 2)],
    }
    
    med_accuracy_df = pd.DataFrame(med_accuracy_dict, index=[index_name2 + ' ' + multiple_name])
    
    mean_mae = np.mean(results_df['{}'.format(mean_err_name)])
    median_mae = np.mean(results_df['{}'.format(median_err_name)])
    
    mean_rmse = np.sqrt(np.mean((results_df['val err mean'])**2))
    median_rmse = np.sqrt(np.mean((results_df['val err median'])**2))
    
    
    
    
    return accuracy_df, med_accuracy_df, mean_mae, mean_rmse, median_mae, median_rmse



def m_check_acc(df1, df2, df3, df4, df5, df6, multiple_name):

    mean_df1_acc, median_df1_acc, mean_mae_df1, mean_rmse_df1, median_mae_df1, median_rmse_df1 = m_interim_accuracy_df(df1, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')
    mean_df2_acc, median_df2_acc, mean_mae_df2, mean_rmse_df2, median_mae_df2, median_rmse_df2 = m_interim_accuracy_df(df2, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')
    mean_df3_acc, median_df3_acc, mean_mae_df3, mean_rmse_df3, median_mae_df3, median_rmse_df3 = m_interim_accuracy_df(df3, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')
    mean_df4_acc, median_df4_acc, mean_mae_df4, mean_rmse_df4, median_mae_df4, median_rmse_df4 = m_interim_accuracy_df(df4, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')
    mean_df5_acc, median_df5_acc, mean_mae_df5, mean_rmse_df5, median_mae_df5, median_rmse_df5 = m_interim_accuracy_df(df5, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')
    mean_df6_acc, median_df6_acc, mean_mae_df6, mean_rmse_df6, median_mae_df6, median_rmse_df6 = m_interim_accuracy_df(df6, multiple_name, 'mean', 'median' ,'abs val err mean', 'abs val err median')

    tot_mean_acc = pd.concat([
        mean_df1_acc,
        mean_df2_acc,
        mean_df3_acc,
        mean_df4_acc,
    ...
    mean_mae_ot = pd.DataFrame(index=['mean {}'.format(multiple_name)]*6)
    mean_rmse_ot = pd.DataFrame(index=['mean {}'.format(multiple_name)]*6)
    median_mae_ot = pd.DataFrame(index=['median {}'.format(multiple_name)]*6)
    median_rmse_ot = pd.DataFrame(index=['median {}'.format(multiple_name)]*6)
    ...
    dates = np.array([valuation_date1, valuation_date2, valuation_date3, valuation_date4, valuation_date5, valuation_date6])
    
    mean_mae_ot['date'] = dates
    mean_rmse_ot['date'] = dates
    median_mae_ot['date'] = dates
    median_rmse_ot['date'] = dates
    
    mean_mae_ot = mean_mae_ot[::-1]
    mean_rmse_ot = mean_rmse_ot[::-1]
    median_mae_ot = median_mae_ot[::-1]
    median_rmse_ot = median_rmse_ot[::-1]
    
    mae_ot = pd.concat([mean_mae_ot, median_mae_ot])
    rmse_ot = pd.concat([mean_rmse_ot, median_rmse_ot])
    
    
    acc_25_df_mean = tot_mean_acc[['% of valuations within 25% of actual price']].reset_index()
    acc_25_df_mean['date'] = dates
    
    acc_25_df_median = tot_median_acc[['% of valuations within 25% of actual price']].reset_index()
    acc_25_df_median['date'] = dates
    
    acc_25_tot = pd.concat([acc_25_df_mean, acc_25_df_median])[::-1]
    acc_25_tot.rename(columns={'index':'multiple'}, inplace=True)
    
    fig1 = plt.figure(figsize=(8,5))
    sns.lineplot(x=acc_25_tot['date'], y=acc_25_tot['% of valuations within 25% of actual price'], data=acc_25_tot, hue=acc_25_tot['multiple'])
    plt.tight_layout()
    plt.close()
    
    fig2, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(12,5))
    sns.lineplot(x='date', y='MAE', data=mae_ot, hue=mae_ot.index, ax=ax1)
    sns.lineplot(x='date', y='RMSE', data=rmse_ot, hue=rmse_ot.index, ax=ax2)
    plt.tight_layout()
    plt.close()
    ...
    mean_tot_bias_error = pd.DataFrame(pd.concat(mean_bias_err_list))
    median_tot_bias_error = pd.DataFrame(pd.concat(median_bias_err_list))
    
    mean_valuation_errors_list = [
         df1['abs val err mean'],
         df2['abs val err mean'],
         df3['abs val err mean'],
         df4['abs val err mean'],
         df5['abs val err mean'],
         df6['abs val err mean'],
    ]
        
    median_valuation_errors_list = [
         df1['abs val err median'],
         df2['abs val err median'],
         df3['abs val err median'],
         df4['abs val err median'],
         df5['abs val err median'],
         df6['abs val err median'],
    ]
     
    mean_total_valuation_errors = pd.DataFrame(pd.concat(mean_valuation_errors_list))
    tot_peers_num = len(mean_total_valuation_errors)
    
    mean_within_10_perc_val_num = len(mean_total_valuation_errors[mean_total_valuation_errors['abs val err mean'] < 0.10])
    mean_within_25_perc_val_num = len(mean_total_valuation_errors[mean_total_valuation_errors['abs val err mean'] < 0.25])
    mean_within_50_perc_val_num = len(mean_total_valuation_errors[mean_total_valuation_errors['abs val err mean'] < 0.50])
    mean_within_75_perc_val_num = len(mean_total_valuation_errors[mean_total_valuation_errors['abs val err mean'] < 0.75])
    mean_within_90_perc_val_num = len(mean_total_valuation_errors[mean_total_valuation_errors['abs val err mean'] < 0.90])
    
    mean_within_10_perc_val_perc = (mean_within_10_perc_val_num / tot_peers_num)*100
    mean_within_25_perc_val_perc = (mean_within_25_perc_val_num / tot_peers_num)*100
    mean_within_50_perc_val_perc = (mean_within_50_perc_val_num / tot_peers_num)*100
    mean_within_75_perc_val_perc = (mean_within_75_perc_val_num / tot_peers_num)*100
    mean_within_90_perc_val_perc = (mean_within_90_perc_val_num / tot_peers_num)*100
    
    mean_accuracy_dict = {
        '% of valuations within 10% of actual price':[round(mean_within_10_perc_val_perc, 2)],
        '% of valuations within 25% of actual price':[round(mean_within_25_perc_val_perc, 2)],
        '% of valuations within 50% of actual price':[round(mean_within_50_perc_val_perc, 2)],
        '% of valuations within 75% of actual price':[round(mean_within_75_perc_val_perc, 2)],
        '% of valuations within 90% of actual price':[round(mean_within_90_perc_val_perc, 2)],
    }
    
    median_total_valuation_errors = pd.DataFrame(pd.concat(median_valuation_errors_list))
    
    median_within_10_perc_val_num = len(median_total_valuation_errors[median_total_valuation_errors['abs val err median'] < 0.10])
    median_within_25_perc_val_num = len(median_total_valuation_errors[median_total_valuation_errors['abs val err median'] < 0.25])
    median_within_50_perc_val_num = len(median_total_valuation_errors[median_total_valuation_errors['abs val err median'] < 0.50])
    median_within_75_perc_val_num = len(median_total_valuation_errors[median_total_valuation_errors['abs val err median'] < 0.75])
    median_within_90_perc_val_num = len(median_total_valuation_errors[median_total_valuation_errors['abs val err median'] < 0.90])
    
    median_within_10_perc_val_perc = (median_within_10_perc_val_num / tot_peers_num)*100
    median_within_25_perc_val_perc = (median_within_25_perc_val_num / tot_peers_num)*100
    median_within_50_perc_val_perc = (median_within_50_perc_val_num / tot_peers_num)*100
    median_within_75_perc_val_perc = (median_within_75_perc_val_num / tot_peers_num)*100
    median_within_90_perc_val_perc = (median_within_90_perc_val_num / tot_peers_num)*100
    
    median_accuracy_dict = {
        '% of valuations within 10% of actual price':[round(median_within_10_perc_val_perc, 2)],
        '% of valuations within 25% of actual price':[round(median_within_25_perc_val_perc, 2)],
        '% of valuations within 50% of actual price':[round(median_within_50_perc_val_perc, 2)],
        '% of valuations within 75% of actual price':[round(median_within_75_perc_val_perc, 2)],
        '% of valuations within 90% of actual price':[round(median_within_90_perc_val_perc, 2)],
    }
    
    mean_tot_acc_df = pd.DataFrame(mean_accuracy_dict, index=['mean' + ' ' + multiple_name])
    median_tot_acc_df = pd.DataFrame(median_accuracy_dict, index=['median' + ' ' + multiple_name])
        
    m_overall_acc = pd.concat([mean_tot_acc_df, median_tot_acc_df])
    
    mean_overall_mae = np.mean(mean_total_valuation_errors.values)
    median_overall_mae = np.mean(median_total_valuation_errors.values)
    
    mean_overall_rmse = np.sqrt(np.mean((mean_tot_bias_error.values)**2))
    median_overall_rmse = np.sqrt(np.mean((median_tot_bias_error.values)**2))
    
    mean_ve_dict = {
        'overall MAE': mean_overall_mae,
        'overall RMSE': mean_overall_rmse,
    }
    
    median_ve_dict = {
        'overall MAE': median_overall_mae,
        'overall RMSE': median_overall_rmse,
    }
    
    mean_ve_df = pd.DataFrame(mean_ve_dict, index=['mean' + ' {}'.format(multiple_name)])
    median_ve_df = pd.DataFrame(median_ve_dict, index=['median' + ' {}'.format(multiple_name)])
    
    ve_overall_df = pd.concat([mean_ve_df, median_ve_df])
    
    return fig1, m_overall_acc, ve_overall_df, fig2, acc_25_tot
...
def compute_ind_m(df):
    ...
    loo = LeaveOneOut()
    for train_index, test_index in loo.split(df_1):

        
        y_train, y_test = df_1[train_index], df_1[test_index]

        median_m = np.median(y_train)
        mean_m = np.mean(y_train)

        median_pred.append(median_m)
        mean_pred.append(mean_m)

    return np.array(median_pred), np.array(mean_pred)
